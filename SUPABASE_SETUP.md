...
- `profiles` / `favorites` / `reports`：登录与云端收藏已在代码中启用（`AUTH_ENABLED = true`），仅在同时配置 Supabase 凭证时生效。

> **安全提示**：`comments` / `verifications` / `reports` / `submissions` 均允许匿名写入，且 anon key 是公开的，任何人都可通过 PostgREST 直接写库（可绕开前端 60s 提交冷却）。若担心刷垃圾/刷票，建议在 Supabase 控制台为这些表开启 **Rate Limiting**，或后续将写入收敛到一个带校验与风控的服务端接口。
...
2. 复制两样：
   ...
   - **anon public key**（新版以 `sb_publishable_` 开头，旧版以 `eyJ` 开头；均为可公开、受 RLS 保护的 key）
