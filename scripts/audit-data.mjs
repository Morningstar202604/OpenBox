 ...
 *   8) 误导标签：type=paid 但 tags 含「免费」相关词
 *   9) 信息缺失：status=dead 但无 tips 说明死亡原因
 *  10) 内容重复：summary 与 description 完全相同（>50字）
 *  11) 数据陈旧：updatedAt 超过60天未更新（status=ok）
 *  12) 命名告警：id 前缀与 subType 不匹配（历史遗留，仅告警不阻断）
 *
...
// 8) type=paid 但 tags 含「免费」相关词（误导标签）
out('\n== 误导标签：type=paid 但 tags 含「免费」相关词 ==');
for (const r of rs) {
  if (r.type === 'paid' && (r.tags || []).some((t) => /免费|白嫖|0元/.test(t))) {
    problems++;
    out(`  ${r.id} [type=paid] tags:${(r.tags || []).join(',')}`);
  }
}

// 9) status=dead 但无 tips 说明死亡原因
out('\n== 信息缺失：status=dead 但无 tips 说明 ==');
for (const r of rs) {
  if (r.status === 'dead' && !r.tips?.trim()) {
    problems++;
    out(`  ${r.id} [${r.subType}] ${r.name}`);
  }
}

// 10) summary 与 description 完全相同（偷懒复制，且超过50字）
out('\n== 内容重复：summary 与 description 完全相同（>50字） ==');
for (const r of rs) {
  if (r.summary && r.description && r.summary === r.description && r.summary.length > 50) {
    problems++;
    out(`  ${r.id} [${r.subType}] (${r.summary.length}字)`);
  }
}

// 11) updatedAt 超过60天未更新（数据可能过时，仅ok状态告警）
out('\n== 数据陈旧：updatedAt 超过60天未更新（status=ok） ==');
const NOW = new Date('2026-09-01');
for (const r of rs) {
  if (r.updatedAt && r.status === 'ok') {
    const diffDays = (NOW - new Date(r.updatedAt)) / (1000 * 60 * 60 * 24);
    if (diffDays > 60) {
      problems++;
      out(`  ${r.id} [${r.subType}] updatedAt=${r.updatedAt} (${Math.floor(diffDays)}天前)`);
    }
  }
}

// 12) id 前缀与 subType 不匹配（历史命名问题，仅告警不阻断）
out('\n== 命名告警：id 前缀与 subType 不匹配（历史遗留，仅告警） ==');
const PREFIX_MAP = {
  'ob-charity-': 'charity',
  'ob-relay-': 'relays',
  'ob-api-': 'free-api',
  'ob-node-': 'proxy-nodes',
  'ob-server-': 'free-server',
  'ob-domain-': 'free-domain',
  'ob-app-': 'ai-apps',
  'ob-tool-': 'tools',
  'ob-learn-': 'learn',
  'ob-model-': 'open-models',
  'ob-agent-': 'ai-agent',
};
let prefixWarn = 0;
for (const r of rs) {
  for (const [prefix, expected] of Object.entries(PREFIX_MAP)) {
    if (r.id.startsWith(prefix) && r.subType !== expected) {
      prefixWarn++;
      out(`  ${r.id} | 前缀期望=${expected} 实际=${r.subType}`);
      break;
    }
  }
}
if (prefixWarn > 0) {
  out(`  (共 ${prefixWarn} 条历史命名问题，改 id 会影响 Supabase 关联数据，仅告警)`);
}
