...
--   3) profiles / favorites / reports 在启用登录后使用（代码中 AUTH_ENABLED=true）。
--      仅当前端同时配置了 Supabase 凭证时，登录/云端收藏才会实际生效。
-- ============================================================
...
--   登录用户亦仅可读取「已通过」投稿。待审/被拒内容仅由管理员经控制台或 SQL 审核，
--   避免把整条审核队列暴露给任意注册用户。
drop policy if exists "submissions_read_auth" on public.submissions;
create policy "submissions_read_auth" on public.submissions for select to authenticated using (status = 'approved');
