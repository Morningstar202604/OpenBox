...
export function ReportModal({ resourceName, resourceId, onClose, onSubmit }: ReportModalProps) {
  ...
  return (
    <Modal open onClose={onClose} ariaLabel={t('report.title')}>
      {done ? (
        ...
        <>
          <ModalHeader title={<h3 className="font-semibold">{t('report.title')}</h3>} onClose={onClose} closeLabel={t('common.close')} />
          ...
          <a
            ...
            href={`https://github.com/Morningstar202604/OpenBox/issues/new?title=${encodeURIComponent(`[反馈] ${resourceName}`)}`}
            target="_blank"
            rel="noreferrer"
          >
